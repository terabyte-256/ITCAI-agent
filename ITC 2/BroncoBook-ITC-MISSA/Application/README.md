...
- Python `3.13`
- [Bun](https://bun.sh/) for frontend tooling and backend script commands
- Optional: [Ollama](https://ollama.com/) if you want to use the local-model path instead of OpenAI

No additional package dependencies have been introduced beyond:

- Python packages in `requirements.txt`
- Bun packages installed by `bun install`
...
- If you want to use OpenAI, set `OPENAI_API_KEY` in `.env`.
- If you want to use Ollama, make sure Ollama is running locally and the configured model is available.
