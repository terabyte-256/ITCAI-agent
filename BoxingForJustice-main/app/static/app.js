...
function renderMetrics() {
  if (metricQueries) metricQueries.textContent = state.totalQueries;
  if (metricUnanswered) metricUnanswered.textContent = state.unanswered;
  if (metricTools) metricTools.textContent = state.toolCalls;
  if (metricSources) metricSources.textContent = state.totalQueries
...
async function loadHealth() {
  try {
    ...
    if (healthStatus) healthStatus.textContent = 'Unavailable';
  }
}
